...
- V4 after-price is still not emitted by the current Base V4 quoter shape. Treat V4 pool hooks, dynamic fees, and custom accounting as a reason to require route-level simulation evidence; do not backfill unverifiable post-swap prices.
...
- Live selected pools: `slipstream-zora-usdc`, `uniswap-v4-basedflick-zora`, `slipstream-basedflick-fame`.
- Live router fee amount: `8843960513186818235`.
- Case `usdc-fame-five-dollars`, amount in `5000000`.
- Live selected pools: `slipstream-zora-usdc`, `slipstream-zora-weth`, `uniswap-v2-fame-direct`.
- Live router fee amount: `44063111907066739789`.
...
- Contract follow-up: add fork regressions for both amounts. The volatile FAME/frxUSD hop now has reserve-based market impact; the stable USDC/frxUSD hop still needs a validated stable-curve state-transition price source before its market impact can be marked computable.

### Prove WETH -> FAME Routes

- Cases: `weth-fame-small-direct`, `weth-fame-split`, `weth-fame-large-closed`.
- Live selected pools: `weth-fame-small-direct` uses `slipstream-zora-weth`, `uniswap-v4-basedflick-zora`, and `slipstream-basedflick-fame`; the larger buckets use `uniswap-v2-fame-direct`.
