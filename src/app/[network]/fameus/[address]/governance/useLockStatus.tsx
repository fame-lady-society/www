...
export const useLockStatus = (chainId: 11155111 | 8453, tokenIds: bigint[]) => {
  ...
  const [guardianAddresses, setGuardianAddresses] = useState<(`0x${string}` | null)[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchLockStatus() {
      ...
      const isLocked = isLockedResponse.map(({ status, result }) => status === "success" ? result : false);
      // get the index of all the true values
      const trueIndices = isLocked.map((isLocked, index) => isLocked ? index : -1).filter((index) => index !== -1);
      ...
      const finalGuardianAddresses = isLocked.map((isLocked, index) => isLocked ? guardianAddressesResponse[index] : null).map(
        (item) => item && item.status === "success" ? item.result : null
      )
