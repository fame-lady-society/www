...
export function useGalleryAuthority(account: Address | null) {
  ...
  const query = useQuery({
    queryKey: account
      ? galleryQueryKeys.authority(identity, blockNumber ?? 0n, account)
      : [...galleryQueryKeys.stack(identity), "authority", "disconnected"],
    queryFn: () => {
      if (!client || !account || blockNumber === null) {
        throw new Error("Wallet account is unavailable");
      }
      return readGalleryAuthority(client, blockNumber, account);
    },
